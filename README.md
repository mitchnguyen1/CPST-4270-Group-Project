...
1.	Ian: Transaction history feature - Allows the user to view the last n transactions they made. This will act as a receipt and will mimic a real banking experience.
...
2. Make sure all Java files (ATM.java, Account.java, OptionMenu.java, etc.) are located in the same folder (ATM/).
3. Open the project in a Java IDE (Eclipse, IntelliJ, NetBeans) or use your command prompt/terminal.
4. In the terminal, cd into your project directory (ATM-Machine).
5. Edit the file named in the resource folder: db.properties.examples and rename it to db.properties (You will need to contact the developer for login info)
6. Compile the project with the PostgreSQL driver on the classpath by running this command:
* Mac: javac -cp "ATM/lib/postgresql-42.7.8.jar" ATM/*.java
* Windows: javac -cp "ATM/lib/postgresql-42.7.8.jar" ATM\*.java
7. Run the ATM program, making sure to include the PostgreSQL jar on the classpath:
* Mac: java -cp "ATM:ATM/lib/postgresql-42.7.8.jar" ATM
* Windows: java -cp "ATM;ATM/lib/postgresql-42.7.8.jar" ATM
